...
func camera_rotpos_applying(delta):
	##Rot Interpolation
	Interpolated_Rot.x = lerp_angle(self.rotation.x, Cam_Virtual_Rot.x, delta * XAxis_Rot_weigth)
	Interpolated_Rot.y = lerp_angle(self.rotation.y, Cam_Virtual_Rot.y, delta * YAxis_Rot_weigth)
	Interpolated_Rot.z = lerp_angle(self.rotation.z, Cam_Virtual_Rot.z, delta * ZAxis_Rot_weigth)
	self.rotation = Interpolated_Rot
	
	##Pos Interpolation
	Interpolated_Pos.x = lerp(self.global_position.x, Cam_Virtual_Pos.x, delta * Horizantal_Pos_weigth)
	Interpolated_Pos.y = lerp(self.global_position.y, Cam_Virtual_Pos.y, delta * Vertical_Pos_weigth)
	Interpolated_Pos.z = lerp(self.global_position.z, Cam_Virtual_Pos.z, delta * Horizantal_Pos_weigth)
	self.global_position = Interpolated_Pos
